            ...
            describe("gdpr") {
                it("build URL with the right parameters") {
                    expect(manager.buildGDPRPmUrl(usedId: usedId, uuid: nil)?.absoluteString)
                        .to(equal("https://\(wrapperHost)/privacy-manager/index.html?consentLanguage=es&idfaStatus=\(idfaStatus)&message_id=1&pmTab=&site_id=\(propertyId)"))
                }
                ...
                describe("when there's uuid") {
                    it("includes the uuid query param") {
                        expect(manager.buildGDPRPmUrl(usedId: usedId, uuid: "gdprUUID")?.absoluteString)
                            .to(equal("https://\(wrapperHost)/privacy-manager/index.html?consentLanguage=es&consentUUID=gdprUUID&idfaStatus=\(idfaStatus)&message_id=1&pmTab=&site_id=\(propertyId)"))
                    }
                }
            }
            ...
            describe("ccpa") {
                it("build URL with the right parameters") {
                    expect(manager.buildCCPAPmUrl(usedId: "1", uuid: nil)?.absoluteString)
                        .to(equal("https://\(wrapperHost)/ccpa_pm/index.html?consentLanguage=es&idfaStatus=\(idfaStatus)&message_id=1&pmTab=&site_id=\(propertyId)"))
                }

                describe("when there's uuid") {
                    it("includes the uuid query param") {
                        expect(manager.buildCCPAPmUrl(usedId: usedId, uuid: "ccpaUUID")?.absoluteString)
                            .to(equal("https://\(wrapperHost)/ccpa_pm/index.html?ccpaUUID=ccpaUUID&consentLanguage=es&idfaStatus=\(idfaStatus)&message_id=1&pmTab=&site_id=\(propertyId)"))
                    }
                }
            }
            ...
            describe("usnat") {
                it("build URL with the right parameters") {
                    expect(manager.buildUSNatPmUrl(usedId: "1", uuid: nil)?.absoluteString)
                        .to(equal("https://\(wrapperHost)/us_pm/index.html?consentLanguage=es&idfaStatus=\(idfaStatus)&message_id=1&pmTab=&site_id=\(propertyId)"))
                }

                describe("when there's uuid") {
                    it("includes the uuid query param") {
                        expect(manager.buildUSNatPmUrl(usedId: usedId, uuid: "usnatUUID")?.absoluteString)
                            .to(equal("https://\(wrapperHost)/us_pm/index.html?consentLanguage=es&idfaStatus=\(idfaStatus)&message_id=1&pmTab=&site_id=\(propertyId)&uuid=usnatUUID"))
                    }
                }
            }
        }
